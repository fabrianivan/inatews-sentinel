...
print("\n" + "="*50)
print("⚡ [6/8] Deploying Tactical Response Generator (flink/06_response_generator.sql)")
print("="*50)
...
# 7. Deploy AI/ML Seismic Forecast
print("\n" + "="*50)
print("🔮 [7/8] Deploying AI/ML Seismic Forecast (flink/07_forecast.sql)")
print("="*50)
with open("flink/07_forecast.sql") as f:
    sql_07 = " ".join([l for l in f.read().splitlines() if not l.strip().startswith("--")])
deploy_statement("job-gempa-forecast", sql_07, is_job=True)

# 8. Deploy Tsunami Wave Forecast
print("\n" + "="*50)
print("🌊 [8/8] Deploying Tsunami Wave Forecast (flink/08_tsunami_forecast.sql)")
print("="*50)
with open("flink/08_tsunami_forecast.sql") as f:
    sql_08 = " ".join([l for l in f.read().splitlines() if not l.strip().startswith("--")])
deploy_statement("job-gempa-tsunami-forecast", sql_08, is_job=True)
